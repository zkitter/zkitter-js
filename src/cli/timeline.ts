...
export function timeline(program: Command) {
  program
    ...
    .action(async options => {
      ...
      const { limit, offset, thread, user } = options;

      const posts = user
        ? await zkitter.getUserPosts(user, limit, offset)
        : thread
        ? await zkitter.getThread(thread, limit, offset)
...
export async function printPosts(zkitter: Zkitter, posts: Post[]) {
  ...
  for (const post of posts) {
    ...
    console.log(
      `${red(nickname)} ${yellowBright(handle)} ${magenta(
        moment(post.createdAt).format('YYYY/MM/DD LTS')
      )}`
    );
